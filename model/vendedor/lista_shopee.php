...
<?php
    ...
    $control->execute();
    $select_type_user=$control->fetch(PDO::FETCH_ASSOC);


    $validation_type=$connection->prepare("SELECT * FROM user INNER JOIN type_user INNER JOIN gender INNER JOIN state ON user.id_type_user = type_user.id_type_user AND user.id_gender=gender.id_gender AND user.id_state=state.id_state AND user.id_type_user=type_user.id_type_user WHERE user.id_type_user=3");
    $validation_type->execute();
    $resultado = $validation_type->fetch(PDO::FETCH_ASSOC);
...
<?php
...

    {
        ...
        $examinar=$connection ->prepare("SELECT * FROM user WHERE document='$cedula' or username='$usuario'");
        $examinar -> execute();
        ...
        else if ($cedula=="" || $nombre==""  || $usuario=="" || $clave=="" || $telefono=="" || $email=="" || $id_gender=="")
        {
        ...
        {
            $register_user=$connection->prepare("INSERT INTO user(document,name,telephone,email,id_type_user,id_gender,password,username,id_state) VALUES('$cedula','$nombre','$telefono','$email','$idusu','$id_gender','$clave', '$usuario','$id_state')" );
				    ...
				    <div class="col-md-12">
					   ...
					   <table class="table table-striped table-hover">
						  ...
						  <tbody>
						  	...
						  	<?php
								foreach($validation_type as $type_shopee){
							...
							?>
									...
									<th>
										<form method="get" action="actualizar_usu.php">
											...
											<input type="hidden" name="documento" value="<?=$type_shopee['document']?>">
									...
									<th><?= $type_shopee["document"]?></th>
									<th><?= $type_shopee["name"]?></th>
                                    <th><?= $type_shopee["surname"]?></th>
									<th><?= $type_shopee["username"]?></th>
									<th><?= $type_shopee["telephone"]?></th>
                                    <th><?= $type_shopee["email"]?></th>
									<th><?= $type_shopee["type_user"]?></th>
									<th><?= $type_shopee["gender"]?></th>
									<th><?= $type_shopee["state"]?></th>
		...
		<div class="modal fade" tabindex="-1" id="addEmployeeModal" role="dialog">
			...
			<div class="container">
				<form method="POST" name="formreg" action="" autocomplete="off" id="formulario" class="formulario">
					...
					<!-- Group Type User and surname -->
					
					<div class="formulario__grupo select" >
						<div class="formulario__grupo__input">
                            <input class="cajas" type="hidden" value="3" name="id_type_user">  
                            <input type="text" class="formulario__input" name="surname" required id="email" placeholder="Ingrese su correo electronico">
