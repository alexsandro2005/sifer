...
<?php
...

    {
        ...
        if ($register_validation){
            ...
            echo '<script> window.location= "crear_cilindraje.php"</script>';
        }
        else if ( $cilindraje=="")
        {
        ...
        }else
        {
            ...
            echo '<script>alert("// Estimado Usuario el registro del nuevo cilindraje ha sido exitoso. //");</script>';
...
<head>
    ...
    <title>CREAR CILINDRAJE || SIFER-APP</title>
...
<body onload="formreg.id_modelos.focus()">
    ...
    <main>
        <div class="container_title">
            <header>CREAR CILINDRAJE</header>
        </div>
        <form method="POST" name="formreg" action="" autocomplete="off" id="formulario" class="formulario">

            <!-- Container: Cilindraje -->

			<div class="formulario__grupo" id="grupo__cilindraje">
				<label for="username" class="formulario__label">Nuevo Cilindraje</label>
				<div class="formulario__grupo-input">
					<input type="number" class="formulario__input" maxlength="4" oninput="maxlengthNumber(this)" name="cilindraje" required id="cilindraje" placeholder="Ingrese el nuevo cilindraje">
					<i class="formulario__validacion-estado fas fa-times-circle"></i>
				</div>
				<p class="formulario__input-error">El nuevo clindraje debe ser de maximo 4 numeros.</p>
			</div>
